...
const markTodoDone = async (id) => {
  ...
  if (!response.ok) {
    throw new Error(`Unable to complete Todo ${id}`);
  }
};
            ...
            .map(
              (todo) => `
                <li style="margin-bottom: 8px;">
                  <span style="${todo.done ? "text-decoration: line-through; color: #666;" : ""}">
                    ${todo.content}
                  </span>
                  ${
                    todo.done
                      ? " <strong>Done</strong>"
                      : `<form method="POST" action="/todos/${todo.id}/done" style="display: inline; margin-left: 8px;">
...
const server = http.createServer(async (req, res) => {
  ...
  const completeTodoMatch = req.url.match(/^\/todos\/(\d+)\/done$/);

  if (req.method === "POST" && completeTodoMatch) {
    await markTodoDone(Number(completeTodoMatch[1]));
    res.writeHead(303, { Location: "/project" });
    res.end();
    return;
  }
